...
var _default = router => {
  ...
  router.post("/payments/:cartId/:cartIndex/with-registered-card-index", _Session.default, async (req, res) => {
    let {
      cardIndex
    } = req.params;
    if (!cardIndex) {
  ...
  router.post("/payments/:cartId/with-registered-card-token", _Session.default, async (req, res) => {
    ...
    if (!cardToken) {
      throw new _ApiError.default("Cart token is requierd", 400, "cardTokenRequired");
    }
    if (!req.user?.cardUserKey) {
      throw new _ApiError.default("No registered card available", 400, "noRegisteredCardAvailable");
    }
